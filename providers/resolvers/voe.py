...
def decode_voe(data):
    data = rot13(data)
    data = clean_separators(data)
    data = remove_underscore(data)
    data = base64.b64decode(data).decode()
    data = shift_decode(data)
    data = data[::-1]
    data = base64.b64decode(data).decode()
    return json.loads(data)


class VoeResolver(BaseResolver):
    ...
    def resolve(self, url: str):
        ...
        print("Scripts JSON encontrados:", len(scripts))

        if not scripts:
            print("No hay JSON cifrado")
            return None

        for script in scripts:
            try:
                content = json.loads(script.text.strip())
                ...
                print("JSON cifrado encontrado")
                print("Descifrando Voe...")

                data = decode_voe(encrypted)
                ...
                if not stream:
                    print("No existe source en los datos")
                    return None
